...
redis_client = redis.StrictRedis(host=redisHost, port=redisPort, db=0)
minioUser = "rootuser"
...
while True:
    try:
        work = redis_client.blpop("toWorkers", timeout=0)
        model_name = (
        ...
        try:
            ...
            print(
                "Recieved source code in location: ", source_code_file_location
            )
            response = minio_client.fget_object(
            ...
            print(
                "Recieved Layer Weights in location: ", layer_weights_file_path
            )
            model = get_model(
